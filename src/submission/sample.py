...
import sys
#sys.path.append("//Users//krithikasaravanan//Desktop//XCS236-DeepGenerativeModels//XCS236-PS1//src//")
...
def temperature_scale(logits, model, new_past, config, temperature, temperature_horizon):
    ...
    if temperature_horizon == 1:
        ...
        joint_probs = F.softmax(scaled_logits, dim=-1)
    ...
    # EXTRA CREDIT ONLY   
        ...
        for t in range(config.vocab_size):
            ...
            joint_prob_t = None
            ### START CODE HERE ###
            # Compute the second-token logits
            with torch.no_grad():
                logits2, _ = model(new_current_text, past=new_past)
                logits2 = logits2[:, -1, :]  # shape: [1, vocab_size]

                # Apply top k filtering for the second token
                logits2 = top_k_logits(logits2, k=config.top_k)

                # Scale by temperature
                logits2_scaled = logits2 / temperature

                # Convert to probabilities
                joint_prob_t = first_prob * F.softmax(logits2_scaled, dim=-1)  
        ...
        first_logits = None
        ### START CODE HERE ###
        # Convert to tensor of shape [num_first_tokens, vocab_size]
        joint_probs = torch.stack(joint_probs, dim=0)  # shape: [num_first_tokens, vocab_size]

        # Marginalize out the second token to get first-token probability
        first_marginal = joint_probs.sum(dim=1)  # shape: [num_first_tokens]

        # Convert back to logits
        first_logits = torch.log(first_marginal + 1e-10)  # avoid log(0)
        ### END CODE HERE ###
...
def sample(model, start_text, config, length, temperature=None, temperature_horizon=1):
    ...
    with torch.no_grad():
        for _ in trange(length):
            ...
            # Update current_text for next step
